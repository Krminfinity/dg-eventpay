...
const ParticipantRegisterPage = () => {
  ...
  useEffect(() => {
    const data = localStorage.getItem('tempJoinData');
    if (data) {
      setJoinData(JSON.parse(data));
    }
  }, []);
  ...
  const handleRegister = async () => {
    ...
    if (!joinData) {
      setError('参加データが見つかりません。最初からやり直してください。');
      return;
    }
    ...
    try {
      ...
      // 一時データクリア
      localStorage.removeItem('tempJoinData');
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ 
          ...
          <div style={{ fontSize: '16px', color: '#666', lineHeight: '1.6' }}>
            <p><strong>{joinData.eventData?.name}</strong></p>
            <p>主催者: {joinData.eventData?.organizer}</p>
            <p>参加費: ¥{joinData.eventData?.fee?.toLocaleString() || '0'}</p>
            <p>参加者名: {joinData.participantName}</p>
          </div>
        </div>
        ...
        <button
          ...
          style={{
            width: '100%',
            padding: '18px',
            backgroundColor: (!loading && password.length >= 6 && password === confirmPassword) ? '#28a745' : '#e9ecef',
